...
async function startServer() {
  ...
  // API Route: Fetch and proxy Google Sheets data to avoid CORS
  app.get('/api/sheets/fetch', async (req, res) => {
    ...
    if (typeof url === 'string' && url.length > 0) {
      ...
      // Default to user's provided sheet
    ...
    try {
      ...
      console.log('Fetching Google Sheet from:', targetUrl);
      ...
      if (!response.ok) {
        ...
        if (gvizResponse.ok) {
          ...
          return res.json({ success: true, csv: csvText, source: gvizUrl });
        }
      ...
      res.json({ success: true, csv: csvText, source: targetUrl });
    } catch (err: any) {

...
import { Download, Copy, Printer, Check, TrendingUp, Loader2, FileDown } from 'lucide-react';
...
export const ReportSlideView: React.FC<ReportSlideViewProps> = ({ reportData, onPrint }) => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const filename = `Visitor_Log_Report_${cleanBranch}_${start}_to_${end}.pdf`;

      const success = await exportElementToPDF('weekly-report-slide-canvas', {
        filename,
  ...
  const handlePrintSlide = async () => {
    ...
    try {
      if (onPrint) {
        ...
        await printElement('weekly-report-slide-canvas', {
          orientation: 'landscape',
          title: `Visitor Log – ${branch} (${formattedDateRange})`
        });
      }
    } catch (err) {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-3.5 rounded-xl border border-slate-200 shadow-sm print:hidden">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-semibold rounded-lg shadow-sm transition-all cursor-pointer ${
              exportSuccess
                ...
                : 'bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-75'
            }`}
          >
            {isExportingPdf ? (
              <>
                <Loader2 className="w-3.5 h-3.5 animate-spin" />
                <span>Generating Slide PDF...</span>
              </>
            ) : exportSuccess ? (
              <>
          ...
          <button
            ...
            disabled={isPrinting}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-700 bg-white hover:bg-slate-50 border border-slate-200 rounded-lg shadow-2xs transition-colors cursor-pointer disabled:opacity-75"
          >
            {isPrinting ? (
              <>
            ...
            <div>
              ...
              <div className="space-y-1 text-xs font-medium text-slate-800">
                {topProducts.slice(0, 10).map((prod) => (
                  <div 
                    key={prod.rank} 
                    className="flex items-center justify-between py-0.5 border-b border-slate-100/70 last:border-0"
                  >
                    <div className="flex items-center gap-1.5 min-w-0 pr-2">
                      <span className="w-5 text-right font-bold text-slate-900 shrink-0">{prod.rank}.</span>
                      <span className="font-semibold text-slate-800 truncate">{prod.category} :</span>
                    </div>
                    <span className="whitespace-nowrap font-bold text-slate-800 shrink-0">
                      {prod.mentions} {prod.mentions === 1 ? 'mention' : 'mentions'}
                    </span>
                  </div>
                ))}
                {topProducts.length === 0 && (
                  <div className="text-xs text-slate-400 py-1">No product mentions recorded</div>
                )}
              </div>
            </div>
